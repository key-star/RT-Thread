...
typedef struct
{
	const char * list_btn_name;
	void * list_btn_value;
}list_btn_info;

list_btn_info list_btn_info_ant_set[] = 
{
	{"AGC", "000"}
};

list_btn_info list_btn_info_ant_det[] = 
{
	{"error code", "000"},
	{"pitch", "000"},
	{"azimuth", "000"},
...
list_btn_info list_btn_info_sat_set[] = 
{
	{"list number", "000"},
	{"longitude", "000"},
	{"symbol rate", "000"},
	{"downlink frequency", "000"},
	{"local oscillator", "000"},
	{"22k", "000"},
	{"voltage", "000"},
	{"polarization", "000"},
};

list_btn_info list_btn_info_gen_set[] = 
{
	{"language", "000"},
	{"factory settings", "000"},
	{"time zone", "000"},
};
 ...
 *  STATIC PROTOTYPES
 **********************/
static void tv_ant_set_create(lv_obj_t * parent);
static void tv_ant_det_create(lv_obj_t * parent);
static void tv_sat_set_create(lv_obj_t * parent);
static void tv_gen_set_create(lv_obj_t * parent);
#if 0
 ...
 **********************/
static lv_obj_t * tv;
static lv_obj_t * tv_ant_set;
static lv_obj_t * tv_ant_det;
static lv_obj_t * tv_sat_set;
static lv_obj_t * tv_gen_set;
static lv_obj_t * kb;

static lv_style_t style_tabview_bg;
static lv_style_t style_list_bg;
static lv_style_t style_list_scrl;
static lv_style_t style_list_btn;
static lv_style_t style_list_label;
...
{
	lv_style_init(&style_tabview_bg);
	lv_style_set_border_width(&style_tabview_bg, LV_STATE_DEFAULT, 0);
	
    tv = lv_tabview_create(lv_scr_act(), NULL);
	lv_obj_align(tv, NULL, LV_ALIGN_CENTER, 0, 0);
	lv_obj_add_style(tv, LV_TABVIEW_PART_BG, &style_tabview_bg);
	
    tv_ant_set = lv_tabview_add_tab(tv, "Ant set");
    tv_ant_det = lv_tabview_add_tab(tv, "Ant det");
    tv_sat_set = lv_tabview_add_tab(tv, "Sat set");
	tv_gen_set = lv_tabview_add_tab(tv, "Gen set");
	
    lv_style_init(&style_list_bg);
	lv_style_set_radius(&style_list_bg, LV_STATE_DEFAULT, 5);
	lv_style_set_bg_color(&style_list_bg, LV_STATE_DEFAULT, LV_COLOR_BLUE);
	lv_style_set_bg_grad_color(&style_list_bg, LV_STATE_DEFAULT, LV_COLOR_BLUE);
	lv_style_set_border_width(&style_list_bg, LV_STATE_DEFAULT, 0);
//	lv_style_set_pad_top(&style_list_bg, LV_STATE_DEFAULT, 0);
//	lv_style_set_pad_bottom(&style_list_bg, LV_STATE_DEFAULT, 0);
//	lv_style_set_pad_left(&style_list_bg, LV_STATE_DEFAULT, 0);
//	lv_style_set_pad_right(&style_list_bg, LV_STATE_DEFAULT, 0);
//	lv_style_set_pad_inner(&style_list_bg, LV_STATE_DEFAULT, 0);
	
	lv_style_init(&style_list_scrl);
	lv_style_set_bg_color(&style_list_scrl, LV_STATE_DEFAULT, LV_COLOR_YELLOW);
	lv_style_set_bg_grad_color(&style_list_scrl, LV_STATE_DEFAULT, LV_COLOR_YELLOW);
	
	lv_style_init(&style_list_btn);
	lv_style_set_bg_color(&style_list_btn, LV_STATE_DEFAULT, LV_COLOR_ORANGE);
	lv_style_set_bg_grad_color(&style_list_btn, LV_STATE_DEFAULT, LV_COLOR_ORANGE);
//	lv_style_set_pad_top(&style_list_btn, LV_STATE_DEFAULT, 0);
//	lv_style_set_pad_bottom(&style_list_btn, LV_STATE_DEFAULT, 0);
//	lv_style_set_pad_left(&style_list_btn, LV_STATE_DEFAULT, 0);
//	lv_style_set_pad_right(&style_list_btn, LV_STATE_DEFAULT, 0);
//	lv_style_set_pad_inner(&style_list_btn, LV_STATE_DEFAULT, 0);
	
	lv_style_set_bg_color(&style_list_btn, LV_STATE_PRESSED, LV_COLOR_GRAY);
	lv_style_set_bg_grad_color(&style_list_btn, LV_STATE_PRESSED, LV_COLOR_GRAY);
	
	lv_style_set_bg_color(&style_list_btn, LV_STATE_DISABLED, LV_COLOR_BLACK);
	lv_style_set_bg_grad_color(&style_list_btn, LV_STATE_DISABLED, LV_COLOR_BLACK);
	
	lv_style_init(&style_list_label);
	lv_style_set_bg_color(&style_list_label, LV_STATE_DEFAULT, LV_COLOR_BLACK);
	lv_style_set_bg_grad_color(&style_list_label, LV_STATE_DEFAULT, LV_COLOR_BLACK);
	
	tv_ant_set_create(tv_ant_set);
	tv_ant_det_create(tv_ant_det);
	tv_sat_set_create(tv_sat_set);
	tv_gen_set_create(tv_gen_set);
...
static void tv_ant_set_create(lv_obj_t * parent)
{
	...
	lv_obj_set_size(list, lv_disp_get_hor_res(NULL) - 30, lv_disp_get_ver_res(NULL) - 80);
	lv_obj_align(list, NULL, LV_ALIGN_CENTER, 0, 0);
	lv_list_set_layout(list, LV_LAYOUT_COLUMN_LEFT);
	lv_cont_set_fit(list, LV_FIT_TIGHT);
	lv_list_set_scrollbar_mode(list, LV_SCROLLBAR_MODE_OFF);
	
	list_btn = lv_list_add_btn(list,NULL,"AGC:");
	lv_btn_set_layout(list_btn, LV_LAYOUT_OFF);
	list_label = lv_label_create(list_btn,NULL);
	lv_obj_align(list_label, NULL, LV_ALIGN_CENTER, 150, 10);
	lv_label_set_text(list_label, "New text");
	
	list_btn = lv_list_add_btn(list, LV_SYMBOL_SAVE, "save&effect");
	list_btn = lv_list_add_btn(list, LV_SYMBOL_CLOSE, "return");
}
...
static void tv_ant_det_create(lv_obj_t * parent)
{
	...
	for(index = 0; index < sizeof(list_btn_info_ant_det) / sizeof(list_btn_info_ant_det[0]); index++)
	{
		list_btn = lv_list_add_btn(list, NULL, list_btn_info_ant_det[index].list_btn_name);
		lv_btn_set_layout(list_btn, LV_LAYOUT_OFF);
		list_label = lv_label_create(list_btn,NULL);
		lv_obj_align(list_label, NULL, LV_ALIGN_CENTER, 150, 10);
		lv_label_set_text(list_label, list_btn_info_ant_det[index].list_btn_value);
	}
...
static void tv_sat_set_create(lv_obj_t * parent)
{
	...
	for(index = 0; index < sizeof(list_btn_info_sat_set) / sizeof(list_btn_info_sat_set[0]); index++)
	{
		list_btn = lv_list_add_btn(list, NULL, list_btn_info_sat_set[index].list_btn_name);
		lv_btn_set_layout(list_btn, LV_LAYOUT_OFF);
		list_label = lv_label_create(list_btn,NULL);
		lv_obj_align(list_label, NULL, LV_ALIGN_CENTER, 150, 10);
		lv_label_set_text(list_label, list_btn_info_sat_set[index].list_btn_value);
	}
	
	list_btn = lv_list_add_btn(list, LV_SYMBOL_SAVE, "save");
	list_btn = lv_list_add_btn(list, NULL, "satellite switch");
	list_btn = lv_list_add_btn(list, LV_SYMBOL_CLOSE, "return");
}
...
static void tv_gen_set_create(lv_obj_t * parent)
{
	...
	for(index = 0; index < sizeof(list_btn_info_gen_set) / sizeof(list_btn_info_gen_set[0]); index++)
	{
		list_btn = lv_list_add_btn(list, NULL, list_btn_info_gen_set[index].list_btn_name);
		lv_btn_set_layout(list_btn, LV_LAYOUT_OFF);
		list_label = lv_label_create(list_btn,NULL);
		lv_obj_align(list_label, NULL, LV_ALIGN_CENTER, 150, 10);
		lv_label_set_text(list_label, list_btn_info_gen_set[index].list_btn_value);
	}
